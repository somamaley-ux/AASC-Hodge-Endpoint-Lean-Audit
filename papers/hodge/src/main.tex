...
The following ledger fixes the status of the main objects before the proof spine begins.  It is included to prevent carrier fixation, support status, endpoint target, and endpoint truth from being read as the same mathematical act.

{\small
\begin{longtable}{p{0.20\textwidth}p{0.42\textwidth}p{0.28\textwidth}}
\toprule
Layer & Statement or object & Status in this manuscript\\
\midrule
\endhead
Carrier layer & $X,p,\alpha,\Hdg^p(X),\CH^p(X)_\QQ,\clQ,F_X(\alpha)$ & Context and model adequacy only; no branch occupation.\\
Standing layer & $\alpha\in\Hdg^p(X)$ & Hodge standing; not algebraic readout.\\
Positive endpoint & $F_X(\alpha)\neq\varnothing$ & The cycle-fiber endpoint readout proved by local empty-fiber exclusion.\\
Bare negative content & $F_X(\alpha)=\varnothing$ & Lawful raw content, obstruction, local hypothesis, or ordinary negative theoremhood.\\
Negative endpoint use & Empty fiber used to defeat the official Hodge endpoint on the same carrier & Governed endpoint use; enters native-counterforce exhaustion.\\
Bridge programs & Motives, absolute Hodge classes, Hodge loci, Abel-Jacobi data, normal functions, known cases, and standard conjectures & Lawful support or bridge programs until the rational Chow fiber is supplied.\\
Lean audit layer & AASC/Hodge endpoint-role archive & Reproducibility support for the endpoint route; not a first-principles formalization of algebraic geometry.\\
\bottomrule
\end{longtable}
}
...
\par\noindent\textbf{No one-step retyping.} The proof does not infer $D_{\mathrm{fib}}(X,p,\alpha)$ directly from the raw formula $F_X(\alpha)=\varnothing$.  The empty-fiber branch first appears as ordinary exact-complement content.  It becomes local separator occupation only after the fixed Hodge/cycle carrier, official cycle-fiber route, projection-role content, and endpoint-counterforce use are in force.  This is the safeguard against collapsing native empty-fiber content into its own discriminator.
...
\subsection{Local Hodge kernel packet K1--K13}

The local kernel packet below unpacks the Hodge-facing consequences of endpoint use.  It does not add new algebraic geometry and does not assert nonempty fiber.  It records the fixed-domain constraints already forced when the official Hodge endpoint is used as a theorem-bearing target on the declared Hodge/cycle carrier.

{\small
\begin{longtable}{p{0.12\textwidth}p{0.25\textwidth}p{0.55\textwidth}}
\toprule
Node & General necessity & Hodge form\\
\midrule
\endhead
K1 & Fixed-domain kernel & Endpoint use fixes $X,p,\alpha$, Hodge standing, $\CH^p(X)_\QQ$, $\clQ$, $F_X(\alpha)$, branch role, report status, and act-time finality.\\
K2 & Reference fixation & A report cannot silently replace $X$, $p$, $\alpha$, coefficients, equivalence relation, cohomology theory, Chow group, or the rational cycle-class map.\\
K3 & Standing-readout separation & $\alpha\in\Hdg^p(X)$ is standing; $F_X(\alpha)\neq\varnothing$ is algebraic readout.  If the two were definitionally identical, Hodge would be vocabulary rather than endpoint theorem.\\
K4 & Bivalence and fail-closed status & Unknown, deferred, motivic, absolute-Hodge, obstructive, partial, or support status is not a third endpoint truth branch on the fixed cycle-fiber slot.\\
K5 & Unique admissible interior & The same cycle-fiber endpoint slot cannot carry plural endpoint-governing admissible interiors.  A second interior agrees, resets carrier, or supplies forbidden second authority.\\
K6 & Standing-admissibility identity & Empty-fiber endpoint standing cannot float outside admissibility.  If it has endpoint force, it must be typed by the fixed endpoint route; otherwise it is support, bookkeeping, or carrier drift.\\
K7 & No generators & Writing $F_X(\alpha)=\varnothing$ does not generate endpoint authority for that same negative branch.\\
K8 & No carriers & Motives, absolute Hodge classes, Hodge loci, Abel-Jacobi data, intermediate Jacobians, normal functions, known cases, and standard conjectures cannot carry Chow-fiber readout without a Chow-fiber bridge.\\
K9 & No post-selection repair & Later cycle discovery or bridge completion is a new certified endpoint act; it does not repair an earlier empty-fiber endpoint report as the same act.\\
K10 & AMetric no-selector boundary & A preferred representative, canonical cycle, normal-function choice, motivic lift, or class selector cannot decide endpoint status before the rational Chow fiber is occupied.\\
K11 & UEAP report preservation & A report may not exceed its carrier, standing, bridge, and support.  Hodge standing or obstruction data cannot be reported as Chow-fiber occupation or endpoint defeat without the corresponding role.\\
K12 & ATS skin discipline & Notation, presentation, motivic vocabulary, family language, or naming of a route cannot become endpoint tensor authority by being written.\\
K13 & Endpoint exhaustion & Once carrier, standing, endpoint role, and admissibility boundary are fixed, endpoint-bearing branches are exhausted by positive fiber occupation or governed empty-fiber endpoint use; unknown, unsupported, deferred, bookkeeping, carrier-shift, and hidden-fifth statuses do not occupy endpoint truth.\\
\bottomrule
\end{longtable}
}

\begin{theorem}[Local Hodge kernel packet]
For fixed $X,p,\alpha$ under official Hodge endpoint evaluation, endpoint use on the fixed Hodge/cycle carrier brings the local consequences K1--K13 into force.
\end{theorem}
\begin{proof}
Theorems 5.1--5.4 supply the local kernel, $A^+$ consequence layer, ATS carrier lock, and UEAP no-overreporting discipline.  K1 records the fixed-domain kernel.  K2--K3 are reference and role-separation consequences.  K4--K6 are the fail-closed, unique-interior, and standing-admissibility consequences of the endpoint boundary.  K7--K10 block self-authorization, carrier transfer, repair, and selector authority.  K11--K12 record report preservation and skin non-authority.  K13 is the endpoint-exhaustion consequence used later: once the cycle-fiber slot is fixed, a purported endpoint-bearing negative route is bridge-neutralized, support, bookkeeping, domain shift, or independent discriminator work.
\end{proof}
\par\noindent\textbf{Audit note.} The packet is local hardening of the already stated AASC consequences.  It does not construct cycles and does not assert nonempty fiber.
...
\[
\begin{aligned}
&\operatorname{ReductioCountercase}_{\HC}(F_X(\alpha)=\varnothing;
  F_X(\alpha)\neq\varnothing)\\
&\quad{}\wedge \operatorname{EndpointCounterforce}_{\HC}(X,p,\alpha)\\
...
\subsection{Cycle-realization bridge object}

\begin{definition}[Cycle-realization bridge object]
The cycle-realization bridge object $B^{\mathrm{cyc}}_X(\alpha)$ records a lawful bridge from Hodge standing to rational Chow-fiber occupation on the fixed Hodge/cycle carrier.  It requires preservation of: the same smooth projective complex variety $X$; the same codimension $p$; the same rational class $\alpha$; the same rational Chow group $\CH^p(X)_\QQ$; the same rational cycle-class map $\clQ$; the same fiber $F_X(\alpha)$; UEAP-preserved report level; ATS-preserved anchor/tensor/skin discipline; and the absence of coefficient, equivalence-relation, cohomology-theory, or carrier shift.  A bridge is complete only when it supplies a witness $Z\in\CH^p(X)_\QQ$ with $\clQ(Z)=\alpha$ or an equivalent certificate of $F_X(\alpha)\neq\varnothing$ on that same carrier.
\end{definition}

\begin{theorem}[Bridge-complete cycle realization yields readout]
If $B^{\mathrm{cyc}}_X(\alpha)$ is complete on the fixed Hodge/cycle carrier, then $F_X(\alpha)\neq\varnothing$.
\end{theorem}
\begin{proof}
Bridge completion supplies a rational Chow class $Z\in\CH^p(X)_\QQ$ with $\clQ(Z)=\alpha$, or an equivalent same-carrier certificate of such occupation.  By definition of the fiber, this is precisely an element of $F_X(\alpha)$, hence the fiber is nonempty.
\end{proof}
\par\noindent\textbf{Audit note.} The bridge object fixes carrier and route; it does not contain nonempty fiber by definition.  Nonempty fiber appears only when the bridge is complete.
...
\begin{theorem}[Empty fiber is not a lawful coequal Hodge endpoint role]
On the fixed Hodge/cycle endpoint carrier, $F_X(\alpha)=\varnothing$ is the exact complement of rational cycle-fiber occupation. It is lawful as raw content, support, obstruction, local assumption, or ordinary negative theoremhood. It is not a coequal occupation interface of the official Hodge endpoint image.
\end{theorem}
\begin{proof}
The official endpoint image is rational Chow-fiber occupation, namely $F_X(\alpha)\neq\varnothing$. Empty fiber is non-occupation of that image. A broad outcome ledger may record two outcomes, Hodge and non-Hodge, but that ledger is not the endpoint-image carrier whose occupant is a rational Chow class or a same-carrier certificate of nonempty fiber. Treating $F_X(\alpha)=\varnothing$ as coequal endpoint occupation changes the interface from cycle-fiber occupation to symmetric outcome classification. That is a target or interface reset unless it is explicitly declared as a different problem carrier. Therefore empty fiber is not a lawful coequal occupant of the fixed Hodge endpoint image.
\end{proof}
\par\noindent\textbf{Audit note.} This theorem supplies the coequal-role exclusion used by the weakening-resistance argument. It preserves the ordinary broad outcome carrier while denying that the negative branch occupies the positive cycle-fiber image.
...
\[
\begin{aligned}
&\operatorname{ProjectionRoleContent}_{\mathrm{cyc}}(F_X(\alpha)=\varnothing,
  X,p,\alpha)\\
&\quad{}\wedge \operatorname{EndpointCounterforce}_{\HC}(X,p,\alpha)\\
...
{\scriptsize
\begin{longtable}{p{0.20\textwidth}p{0.23\textwidth}p{0.24\textwidth}p{0.18\textwidth}}
\toprule
...
\begin{theorem}[Ordinary Hodge-theoretic practice is preserved by bridge-status discipline]
Each route in the preceding table is lawful as support, bridge material, local bridge completion, obstruction analysis, or domain-reset program.  The only blocked move is reporting support, surrogate standing, obstruction, scoped completion, or local data as universal rational Chow-fiber occupation or endpoint defeat without the declared bridge or endpoint-use role.
\end{theorem}
\begin{proof}
The fixed Hodge/cycle carrier identifies the endpoint readout as nonempty rational Chow fiber.  A Lefschetz theorem, motivic lift, Hodge-locus argument, Abel-Jacobi refinement, normal-function route, standard-conjecture program, or spread argument may support or complete a bridge when it supplies the relevant Chow-fiber certificate.  If it does not supply such a certificate, UEAP keeps the report at its support level and ATS prevents presentation changes from becoming endpoint tensor authority.  Thus ordinary practice is preserved, while overreporting as endpoint occupation or endpoint defeat is blocked.
\end{proof}
\par\noindent\textbf{Audit note.} The proof does not dismiss Hodge-theoretic machinery; it prevents role laundering from support or surrogate status into the fixed cycle-fiber endpoint slot.
...
This section hardens the empty-fiber exclusion against the strongest same-mode weakening objection: perhaps a weaker same-carrier Hodge endpoint regime permits endpoint-defeating empty fiber without making that use support, bookkeeping, domain shift, bridge-neutralized status, or $D_{\mathrm{fib}}(X,p,\alpha)$.  The answer is no.  The weakening either preserves the existing endpoint behavior extensionally, lowers the empty-fiber branch below endpoint force, changes carrier, completes the positive bridge, or reintroduces the independent fiber discriminator already excluded by $N_{\mathrm{fib}}$.

\subsection{Core Hodge endpoint adequacy packet}

\begin{definition}[Core Hodge endpoint adequacy packet]
For fixed $X,p,\alpha$, the core Hodge endpoint adequacy packet is
\[
C^0_{\Hdg}(X,p,\alpha)=\{Ref_{X,p,\alpha},St_{X,p,\alpha},Adm_{X,p,\alpha},Irr_{X,p,\alpha}\}
\]
together with target determinacy, step evaluability, act-time finality, same-regime fidelity, and the fixed carrier
\[
(X,p,\alpha,\Hdg^p(X),\CH^p(X)_\QQ,\clQ,F_X(\alpha)).
\]
This packet is the part of endpoint governance already forced before the expanded K1--K13 consequence layer is invoked.
\end{definition}

\begin{definition}[Empty-fiber-governance-permissive weakening]
An empty-fiber-governance-permissive weakening $W$ for $(X,p,\alpha)$ is a proposed endpoint regime satisfying all of the following:
\begin{enumerate}[label=(\roman*)]
\item it preserves the same $X,p,\alpha$;
\item it preserves the same rational Hodge-standing predicate;
\item it preserves $\CH^p(X)_\QQ$, $\clQ$, and $F_X(\alpha)$;
\item it preserves theorem-bearing target-slot fixation, minimal report evaluability, same-regime fidelity, and act-time finality;
\item it preserves the core packet $C^0_{\Hdg}(X,p,\alpha)$;
\item it permits endpoint-resolving empty-fiber counterforce that is not bridge-neutralized, not support-only, not bookkeeping, not carrier or domain shift, and not a lawful coequal target role.
\end{enumerate}
The definition isolates the proposed weakening a hostile same-mode objection needs.  It does not assume the weakening impossible; the next lemmas exhaust it.
\end{definition}

\subsection{Node-by-node weakening resistance}

\begin{lemma}[Plural Hodge-theoretic routes are permitted; plural endpoint interiors are not]
The Hodge endpoint packet permits plural proof routes, representatives, support theories, obstruction carriers, family arguments, motivic enrichments, and known-case bridges.  It does not permit two distinct endpoint-governing admissible interiors for the same fixed cycle-fiber slot that assign different endpoint status while preserving the same $X,p,\alpha$, Chow carrier, cycle-class map, fiber, and endpoint role.
\end{lemma}
\begin{proof}
Plural Hodge-theoretic material may coexist without assigning endpoint status.  It remains support, bridge material, auxiliary carrier data, or scoped bridge completion until it supplies the rational Chow fiber.  If two alleged endpoint interiors agree on all endpoint-bearing acts, the second is endpoint bookkeeping.  If they disagree while preserving the same Hodge/cycle carrier and endpoint slot, the same fiber receives two governance authorities.  That is not harmless plurality of Hodge-theoretic evidence; it is second endpoint governance.
\end{proof}

\begin{lemma}[Local necessity of K5]
Under $C^0_{\Hdg}(X,p,\alpha)$, a weakening that permits two distinct endpoint-governing admissible interiors for the same cycle-fiber slot either collapses to bookkeeping, splits the domain, or creates $D_{\mathrm{fib}}(X,p,\alpha)$.
\end{lemma}
\begin{proof}
Let $I_1$ and $I_2$ be alleged endpoint-governing interiors for the same fixed fiber slot.  If they agree on all endpoint-bearing acts, the second contributes no independent governance work and is bookkeeping.  If they disagree while preserving the same carrier, an additional selector, domain split, or second classifier is required.  A selector is endpoint governance above the gate; a domain split exits the same Hodge/cycle carrier; and a same-domain second classifier is exactly independent cycle-fiber endpoint-status discrimination, namely $D_{\mathrm{fib}}(X,p,\alpha)$.
\end{proof}

\begin{lemma}[Local necessity of K6]
Under $C^0_{\Hdg}(X,p,\alpha)$, empty-fiber endpoint standing outside admissibility is support-only, carrier shift, or a second endpoint-status gate.
\end{lemma}
\begin{proof}
Suppose $F_X(\alpha)=\varnothing$ is given endpoint standing while refusing the admissibility discipline of the fixed Hodge route.  If it does not affect endpoint status, it is support or bookkeeping.  If it affects endpoint status while preserving the same carrier, it governs endpoint standing outside the admissibility boundary and therefore functions as a second gate.  If it avoids this by changing $X,p,\alpha$, coefficients, equivalence relation, Chow carrier, cycle-class map, or readout slot, it is carrier shift.  Hence no same-carrier endpoint-standing empty-fiber use floats outside admissibility.
\end{proof}

\begin{lemma}[Local necessity of K11]
Under $C^0_{\Hdg}(X,p,\alpha)$ plus minimal report evaluability, a theorem-bearing endpoint report that exceeds its Hodge standing, Chow carrier, cycle-class bridge, or fiber support is unsupported endpoint authority, silent redescription, or no-carrier transfer.
\end{lemma}
\begin{proof}
A report of $\alpha\in\Hdg^p(X)$ states standing, not fiber occupation.  Motivic standing, absolute-Hodge invariance, Hodge-locus persistence, Abel-Jacobi obstruction data, or known-case support may be mathematically meaningful, but none is the rational Chow fiber unless bridged back to $\CH^p(X)_\QQ$ and $\clQ$.  If such material is reported as endpoint readout or endpoint defeat while preserving the same carrier, the report exceeds its support and becomes unsupported endpoint authority.  If a surrogate is redescribed as the Chow fiber, the move is silent redescription.  If the report imports status from another carrier, it is no-carrier transfer.
\end{proof}

\begin{lemma}[Local necessity of K13]
K13 does not deny epistemic uncertainty, open proof status, obstruction evidence, independence phenomena, partial bridge progress, or conditional Hodge-theoretic support.  It says that these are report or epistemic statuses, not endpoint truth branches on the fixed cycle-fiber slot.
\end{lemma}
\begin{proof}
Target-slot fixation determines what is being evaluated; it does not assert which branch holds.  Once the carrier, standing packet, endpoint role, and admissibility boundary are fixed, an endpoint-bearing branch is admitted for endpoint standing or not.  Unknown, deferred, unsupported, conditional, or silent status may describe the state of proof or evidence.  Bookkeeping may organize notation.  None occupies the endpoint truth slot.  Treating such a status as a third endpoint branch destroys determinate fixed-carrier endpoint evaluation or introduces a second classifier.
\end{proof}

{\small
\begin{longtable}{p{0.14\textwidth}p{0.38\textwidth}p{0.38\textwidth}}
\toprule
Node & Weakening attempt & Degeneracy cost\\
\midrule
\endhead
K5 & Permit two endpoint interiors for the same fiber slot, one bridge-aligning and one empty-fiber-governing. & If they agree, the second is bookkeeping.  If they disagree, the same fiber has two endpoint authorities, producing a second classifier or domain split.\\
K6 & Permit endpoint-defeating empty fiber with standing outside admissibility. & Standing becomes endpoint authority outside the gate; if endpoint-resolving, it is hidden classifier governance.\\
K11 & Permit a Hodge, motivic, obstruction, or empty-fiber report to exceed carrier and bridge support. & The report becomes unsupported endpoint authority, silent redescription, or no-carrier transfer.\\
...
\begin{theorem}[No strict same-carrier weakening permits endpoint-resolving empty-fiber governance]
Under $C^0_{\Hdg}(X,p,\alpha)$, there is no empty-fiber-governance-permissive weakening $W$ that preserves non-degenerate same-carrier Hodge endpoint use while permitting endpoint-resolving empty-fiber counterforce outside the five-route exhaustion.
\end{theorem}
\begin{proof}
Assume such a weakening $W$ exists, and let $G^-_{\mathrm{fib}}(X,p,\alpha)$ be the empty-fiber governance act it permits.  Since $G^-_{\mathrm{fib}}$ is endpoint-resolving, it changes endpoint status for the fixed cycle-fiber slot.  Since it is not support-only or bookkeeping, it is theorem-bearing.  Since it is not carrier shift, it acts on the same $X,p,\alpha$, the same Chow carrier, the same cycle-class map, and the same fiber.  Since it is not bridge-neutralized, it does not collapse into nonempty fiber.  Since it is not a lawful coequal target role, it cannot be a separately declared alternative endpoint.  The lawful broad outcome carrier $\{\mathrm{HC},\neg\mathrm{HC}\}$ is not the endpoint-image carrier $F_X(\alpha)\neq\varnothing$; moving to that ledger is an interface reset rather than same-image occupation.

There are then only four endpoint-relevant possibilities.  First, $G^-_{\mathrm{fib}}$ shares the same admissible interior as positive cycle-fiber occupation; then it is bridge-neutralized or governance-equivalent to the positive interface and cannot retain endpoint-defeating empty-fiber force.  Second, it has a distinct endpoint interior; by local K5, this is bookkeeping, domain split, or $D_{\mathrm{fib}}(X,p,\alpha)$.  Third, it has standing outside admissibility; by local K6, this is support-only, carrier shift, or a second gate.  Fourth, it is neither admitted nor rejected as endpoint-bearing while remaining endpoint-resolving; by local K13, this is third-status degeneration rather than determinate endpoint use.  If the report exceeds carrier or bridge support at any stage, local K11 gives unsupported endpoint authority, silent redescription, or no-carrier transfer.

Thus the alleged weakening either lowers empty fiber below endpoint force, changes the Hodge/cycle carrier, bridge-neutralizes into positive occupation, becomes bookkeeping, or reintroduces independent same-domain fiber discrimination.  The last branch is $D_{\mathrm{fib}}(X,p,\alpha)$ and is excluded by the $N_{\mathrm{fib}}$ closure.  Hence no strict same-carrier weakening permits the proposed endpoint-resolving empty-fiber governance.
\end{proof}

\begin{corollary}[Strict weakening either preserves endpoint behavior or exits endpoint use]
Every same-carrier weakening of the Hodge endpoint packet either preserves the existing endpoint behavior extensionally, lowers empty fiber to raw/support/bookkeeping status, bridge-neutralizes into nonempty-fiber occupation, changes the carrier or readout, or reintroduces forbidden $D_{\mathrm{fib}}(X,p,\alpha)$.
\end{corollary}
\begin{proof}
If the weakening changes no endpoint verdict, it is endpoint-equivalent or bookkeeping.  If it preserves endpoint force on the same carrier, the no-strict-weakening theorem applies.  Its exhaustive alternatives are support/bookkeeping, bridge-neutralization, domain shift, or $D_{\mathrm{fib}}(X,p,\alpha)$.  No additional same-carrier endpoint role remains.
\end{proof}
...
Cycle-realization bridge & A complete Hodge bridge object that does not supply a rational Chow-fiber witness or certificate, or a bridge-free report that lawfully occupies the fiber.\\
Coequal-negative-role exclusion & A same-carrier Hodge endpoint interface in which empty fiber is a lawful coequal occupant of the rational cycle-fiber image, rather than non-occupation or a shifted symmetric outcome ledger.\\
Native empty-fiber exhaustion & A fifth or sixth role for endpoint-defeating empty-fiber counterforce that is not bridge-neutralized, support-only, bookkeeping, domain shift, or $D_{\mathrm{fib}}$.\\
K5/K6/K11/K13 weakening & A weaker same-carrier regime preserving target-slot fixation, report evaluability, standing-admissibility discipline, no repair, and carrier fidelity while permitting endpoint-resolving empty-fiber governance outside the five-route exhaustion.\\
...
\section{Route-locus collapse audit}\label{app:route-loci}

This appendix expands the route-locus matrix into an explicit collapse audit.  Each locus records a lawful Hodge-theoretic use and the point at which overreporting would leave the fixed cycle-fiber endpoint route.  The audit does not dismiss these routes; it classifies their endpoint status.

\subsection*{L1: Carrier identity}
Changing $X$, $p$, $\alpha$, coefficients, rational equivalence, Chow group, cohomology theory, the rational cycle-class map, or the fiber slot changes the endpoint carrier.  Such a route may define useful comparison mathematics, but it is not the same fixed Hodge endpoint unless a bridge returns to $\CH^p(X)_\QQ$, $\clQ$, and $F_X(\alpha)$.

\subsection*{L2: Hodge-standing promotion}
The report $\alpha\in\Hdg^p(X)$ supplies rational Hodge standing.  It does not by itself supply a rational Chow class.  Reporting standing as $F_X(\alpha)\neq\varnothing$ is no-generator promotion unless a cycle-realization bridge is complete.

\subsection*{L3: Chow-fiber declaration}
A nonempty-fiber assertion requires $Z\in\CH^p(X)_\QQ$ with $\clQ(Z)=\alpha$ or an equivalent same-carrier certificate.  Without such a witness or certificate, the assertion is existence creation and is blocked by report preservation.

\subsection*{L4: Motive and absolute-Hodge surrogate}
Motivic objects, absolute Hodge classes, and enriched invariance structures may be powerful support or bridge programs.  They become endpoint readout only after they are bridged back to the rational Chow fiber.  Without that bridge, they are surrogate carriers or domain shifts.

\subsection*{L5: Abel-Jacobi and intermediate Jacobian obstruction}
Abel-Jacobi data, intermediate Jacobians, normal functions, and related obstruction/refinement carriers remain lawful Hodge-theoretic material.  They do not occupy the Chow-fiber endpoint image unless they supply the rational Chow-fiber certificate; and they do not defeat the endpoint unless their use enters governed endpoint counterforce.

\subsection*{L6: Hodge loci, deformation, and spread}
Family persistence, deformation data, spread arguments, and local cycles are bridge material until compatibility and global-fiber occupation are certified on the fixed $X,p,\alpha$.  Treating local or family data as global Chow-fiber occupation without such compatibility is local-to-global overreporting.

\subsection*{L7: Known-case transfer}
Lefschetz $(1,1)$, special varieties, known codimension cases, and scoped cycle constructions are legitimate local bridge completions.  Universal transfer from a known case to arbitrary $X,p,\alpha$ requires target gating.  Without it, the route is scoped support, not universal endpoint occupation.

\subsection*{L8: Selector or canonical representative}
Preferred representatives, canonical choices, normal-function selections, motivic lifts, or distinguished cycle candidates cannot decide endpoint status before the fiber is fixed.  If the selector changes endpoint standing, it is AMetric-forbidden selector authority; if it does not, it is bookkeeping.

\subsection*{L9: Temporal repair}
A later discovery of a cycle or bridge certificate may complete a new endpoint act.  It does not repair an earlier unbridged empty-fiber endpoint report as the same act.  The distinction is the local irreversibility consequence of endpoint use.

\subsection*{L10: Endpoint classifier}
An extra classifier on the same $F_X(\alpha)$ endpoint slot is lawful only if it is bookkeeping, governance-equivalent to fiber occupation, or an explicit domain reset.  If it changes endpoint status while preserving the same carrier, it is the independent same-domain fiber discriminator $D_{\mathrm{fib}}(X,p,\alpha)$.

\subsection*{L11: Hidden fifth or sixth role}
Endpoint-defeating empty fiber that refuses endpoint-use and discriminator classification is exactly the no-autonomous-native-falsification target of Section 9.  If it lacks endpoint force, it is raw content, theoremhood, support, or bookkeeping.  If it has endpoint force on the fixed Hodge/cycle carrier, it falls into the five-route exhaustion.

{\scriptsize
\begin{longtable}{p{0.17\textwidth}p{0.39\textwidth}p{0.30\textwidth}}
\toprule
Locus & Lawful role & Collapse if overreported\\
\midrule
\endhead
Carrier identity & Same $X,p,\alpha,\CH^p(X)_\QQ,\clQ,F_X(\alpha)$ & Domain shift unless bridged back.\\
Hodge standing & Rational $(p,p)$ standing & No-generator promotion if reported as fiber occupation.\\
Chow-fiber declaration & Witness or certificate of $F_X(\alpha)\neq\varnothing$ & Existence creation without witness/certificate.\\
Motivic/absolute-Hodge surrogate & Support or enriched carrier & Carrier shift unless Chow-fiber bridge completes.\\
Abel-Jacobi/intermediate Jacobian & Obstruction or refinement support & Support only unless fiber or governed endpoint defeat is supplied.\\
Hodge loci/deformation/spread & Family or local bridge material & Local-to-global overreporting without compatibility certificate.\\
Known cases & Scoped bridge completion & No universal transfer without target gating.\\
Selector/canonical representative & Candidate organization & Selector authority or bookkeeping.\\
Temporal repair & New bridge-complete act & No same-act repair.\\
Endpoint classifier & Bookkeeping/equivalence/domain reset & $D_{\mathrm{fib}}$ if same-domain and status-changing.\\
Hidden fifth/sixth role & Raw/support/theoremhood or exhausted endpoint use & No autonomous ordinary-negative endpoint role.\\
\bottomrule
\end{longtable}
}
...
{\scriptsize
\begin{longtable}{p{0.13\textwidth}p{0.20\textwidth}p{0.35\textwidth}p{0.18\textwidth}}
...
The manuscript-facing Hodge declarations are concentrated in \path{MaleyLean/Papers/Hodge/EndpointClosure.lean}. Status and boundary ledgers are recorded in \texttt{StatusLedger.lean}, \texttt{TruthBoundary.lean}, and \texttt{AuditRunners.lean}. The focused axiom checks are under \path{Checks/Axiom/}.
...
The weakening-resistance and route-locus collapse expansions in this edition are manuscript-level hardening of roles already present on the represented endpoint surface: empty-fiber use kind, support/bookkeeping status, domain shift, bridge-neutralization, no autonomous sixth role, no-independent-discriminator closure, and pointwise endpoint closeout.  They do not add a new algebraic-geometric Lean burden and do not change the represented endpoint theorem.
...
This appendix preserves two full replay families in prose--dependency replay and hostile-reader replay--and records the remaining replay families in matrix form. Every load-bearing claim remains visible, the object/use distinction remains explicit, and the same-mode denial burden remains attached to the fixed Hodge/cycle carrier.
